...
local function lock_group_msgbot(msg, data, target)
  ...
  local group_msgbot_lock = data[tostring(target)]['settings']['lock_msgbot']
  if group_msgbot_lock == '✅' then
    return 'msgbot is already locked'
  else
    data[tostring(target)]['settings']['lock_msgbot'] = '✅'
    save_data(_config.moderation.data, data)
...
local function unlock_group_msgbot(msg, data, target)
  ...
  if group_msgbot_lock == '❌' then
    ...
    data[tostring(target)]['settings']['lock_msgbot'] = '❌'
    save_data(_config.moderation.data, data)
    return 'msgbot has been unlocked'
  end
end
...
local function lock_group_heavy(msg, data, target)
  ...
  local group_heavy_lock = data[tostring(target)]['settings']['lock_heavy']
  if group_heavy_lock == '✅' then
    return 'heavy is already locked'
  else
    data[tostring(target)]['settings']['lock_heavy'] = '✅'
    save_data(_config.moderation.data, data)
...
local function lock_group_alliance(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_etehad_lock = data[tostring(target)]['settings']['alliance']
  if group_etehad_lock == '✅' then
    return 'alliance setting is already locked'
  else
    data[tostring(target)]['settings']['alliance'] = '✅'
    save_data(_config.moderation.data, data)
    return 'alliance setting has been locked'
  end
end

local function unlock_group_alliance(msg, data, target)
  if not is_momod(msg) then
    return
  end
  local group_alliance_lock = data[tostring(target)]['settings']['alliance']
  if group_alliance_lock == '❌' then
    return 'alliance setting is not locked'
  else
    data[tostring(target)]['settings']['etehad'] = '❌'
    save_data(_config.moderation.data, data)
    return 'alliance setting has been unlocked'
  end
end
...
function show_supergroup_settingsmod(msg, target)
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['lock_easy'] then
			data[tostring(target)]['settings']['lock_easy'] = '❌'
		end
	end
	  if data[tostring(target)]['settings'] then
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['lock_heavy'] then
			data[tostring(target)]['settings']['lock_heavy'] = '❌'
		end
	end
	  if data[tostring(target)]['settings'] then
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['lock_msgbot'] then
			data[tostring(target)]['settings']['lock_msgbot'] = '❌'
		end
	end
	if data[tostring(target)]['settings'] then
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['alliance'] then
			data[tostring(target)]['settings']['alliance'] = '❌'
		end
	end
  ...
  local text = "⚙SuperGroup Settings:\n📃Lock #links: "..settings.lock_link.."\n👤Lock #contacts "..settings.lock_contacts.."\n💬Lock #arabic: "..settings.lock_arabic.."\n👥Lock #member: "..settings.lock_member.."\n✏Lock #rtl: "..settings.lock_rtl.."\n📎Lock #tgservice: "..settings.lock_tgservice.."\n📝Lock #msgbot: "..settings.lock_msgbot.."\n🎭Lock #sticker: "..settings.lock_sticker.."n📠Lock #tag: "..settings.lock_tag.."\nLock #username: "..lock_username.."\n😜Lock #emoji: "..settings.lock_emoji.."\n🔡Lock #english: "..settings.lock_english.."n📨Lock #fwd: "..settings.lock_fwd.."\n📄Lock #reply: "..settings.lock_reply.."\n🏃Lock #join: "..settings.lock_join.."\n📁Lock #media: "..settings.lock_media.."\🔊Lock #voice: "..lock_voice.."\n🎥Lock #video: "..lock_video.."\n🤐Lock #badw: "..settings.lock_badw.."\n🚶Lock #leave: "..settings.lock_leave.."\n🤖Lock #bots: "..bots_protection.."\n📟Lock #operator: "..settings.operator.."\n😈Lock #spam:"..settings.lock_spam.."\n💢Lock #strict_settings: "..settings.strict.."\n📑Lock #flood: "..NUM_MSG_MAX.."\n➖➖➖➖➖➖➖\n🔴Switch #alliance: "..settings.alliance.."\n🔵Switch #normal: "..settings.normal.."\n⚫Switch #family: "..settings.family.."\n➖➖➖➖➖➖➖\n🔸Traffic #easy: "..settings.lock_easy.."\n🔹Traffic #heavy: "..settings.lock_heavy.."\n📰Group #public: "..settings.public.."\n👾Group #model: "..gp_type.."\n♻Lock #all: "..settings.all
  return text
end
...
local function run(msg, matches)
   ...
   unlock_group_video(msg, data, target),
		...
		unlock_group_operator(msg, data, target),
      	...
      	return unlock_group_etehad(msg, data, target), detehad
			...
			if matches[2] == 'family' then
				...
				return unlock_group_family(msg, data, target)
			end
			if matches[2] == 'fwd' then
		...
		if matches[1] == 'help' and not is_owner(msg) then
			text = "Message /superhelp to @Kiava_bots in private for SuperGroup help"
